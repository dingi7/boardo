...
export const endpoints = {
  ...
  //notifications
  notificationsMarkRead: (notificationId: string | null) =>
    notificationId
      ? `/auth/notifications/${notificationId}`
      : '/auth/notifications/all',
  ...
  banMemberFromBoard: '/auth/orgs/banMember',
...
export const getStatisticsForOrg = async (
...
export const banMemberFromBoard = async (
  orgId: string | undefined,
  memberId: string
): Promise<any> => {
  return api.post(endpoints.banMemberFromBoard, { 
    orgId,
    memberId 
});
};
